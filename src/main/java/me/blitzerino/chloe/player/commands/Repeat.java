                ...
                if(getAPI().hasRank(e.getChannel(), e.getAuthor())) {
                    if (player.isStopped()) {
                        if (player.getPreviousAudioSource() != null) {
                            player.reload(true);
                            getAPI().sendmsg(e.getChannel(), "The previous song has been restarted.", true);
                        } else {
                            getAPI().sendmsg(e.getChannel(), "The player has never played a song, so it cannot restart a song.", true);
                        }
                    } else {
                        player.reload(true);
                        getAPI().sendmsg(e.getChannel(), "The currently playing song has been restarted!", true);
                    }
                }else{
