        ...
        if (e.getMessage().getContent().startsWith(getAPI().getCmdStart())) {
            ...
            if (args[0].equalsIgnoreCase("volume")) {
                if(getAPI().hasRank(e.getChannel(),e.getAuthor())) {
                    if (args.length == 1) {
                        getAPI().sendmsg(e.getChannel(), "The current volume is set at " + String.valueOf(player.getVolume()) + ".", true);
                        return;
                    }
                    float volume = Float.parseFloat(message.substring("volume ".length()));
                    volume = Math.min(1F, Math.max(0F, volume));
                    player.setVolume(volume);
                    getAPI().sendmsg(e.getChannel(), "Volume was changed to: " + volume, true);
                }else{
