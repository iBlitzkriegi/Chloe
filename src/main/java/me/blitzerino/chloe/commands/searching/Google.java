            ...
            if (args[0].equalsIgnoreCase("google")) {
                if (args.length >= 2) {
                    if (e.getMessage().getMentionedUsers().size() == 0) {
                        ...
                        getAPI().sendmsg(e.getChannel(),"You can only include one user!" , false);
                    }
                }else{
                    getAPI().sendmsg(e.getChannel(), "Invalid amount of arguments, try again " + e.getAuthor().getAsMention() + ".", false);
                }
            }
        }
    }
